...
export const deleteBlogBookmarkHandler = async (
  ...
  Response<
      BlogBookmarkServiceReturnType<"deleteBlogBookmark">
  >
> => {
  try {
    const { data: validatedData, error } = DeleteBlogBookmarkSchema.safeParse(Object.fromEntries(formData.entries()));
    if (error)
      return failure("Invalid ID format");
    const blogBookmarkService = new BlogBookmarkServiceImplementation(prisma);
    return { success: true, data: await blogBookmarkService.deleteBlogBookmark(validatedData.id) };
  } catch (error) {
