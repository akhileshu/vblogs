...
export const deleteBlogContentHandler = async (
  ...
  Response<
      BlogContentServiceReturnType<"deleteBlogContent">
  >
> => {
  try {
    const { data: validatedData, error } = DeleteBlogContentSchema.safeParse(Object.fromEntries(formData.entries()));
    if (error)
      return failure("Invalid ID format");
    const blogContentService = new BlogContentServiceImplementation(prisma);
    return { success: true, data: await blogContentService.deleteBlogContent(validatedData.id) };
  } catch (error) {
