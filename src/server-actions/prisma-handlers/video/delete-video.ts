...
export const deleteVideoHandler = async (
  ...
  Response<
      VideoServiceReturnType<"deleteVideo">
  >
> => {
  try {
    const { data: validatedData, error } = DeleteVideoSchema.safeParse(Object.fromEntries(formData.entries()));
    if (error)
      return failure("Invalid ID format");
    const videoService = new VideoServiceImplementation(prisma);
    return { success: true, data: await videoService.deleteVideo(validatedData.id) };
  } catch (error) {
