...
export const deleteUserHandler = async (
  prevState: unknown,
  formData: FormData
): Promise<Response<UserServiceReturnType<"deleteUser">>> => {
  try {
    const { data: validatedData, error } = DeleteUserSchema.safeParse(
    ...
    return {
      success: true,
      data: await userService.deleteUser(validatedData.id),
    };
  } catch (error) {
