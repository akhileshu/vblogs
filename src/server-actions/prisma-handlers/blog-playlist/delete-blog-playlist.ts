...
export const deleteBlogPlaylistHandler = async (
  ...
  Response<
      BlogPlaylistServiceReturnType<"deleteBlogPlaylist">
  >
> => {
  try {
    const { data: validatedData, error } = DeleteBlogPlaylistSchema.safeParse(Object.fromEntries(formData.entries()));
    if (error)
      return failure("Invalid ID format");
    const blogPlaylistService = new BlogPlaylistServiceImplementation(prisma);
    return { success: true, data: await blogPlaylistService.deleteBlogPlaylist(validatedData.id) };
  } catch (error) {
