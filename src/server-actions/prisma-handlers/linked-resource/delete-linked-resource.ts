...
export const deleteLinkedResourceHandler = async (
  ...
  Response<LinkedResourceServiceReturnType<"deleteLinkedResource">>
> => {
  try {
    const { data: validatedData, error } = DeleteLinkedResourceSchema.safeParse(
    ...
    return {
      success: true,
      data: await linkedResourceService.deleteLinkedResource(validatedData.id),
    };
  } catch (error) {
