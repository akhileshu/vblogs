...
export const deleteBlogReadHistoryHandler = async (
  ...
  Response<BlogReadHistoryServiceReturnType<"deleteBlogReadHistory">>
> => {
  try {
    const { data: validatedData, error } =
      DeleteBlogReadHistorySchema.safeParse(
    ...
    return {
      success: true,
      data: await blogReadHistoryService.deleteBlogReadHistory(
        validatedData.id
      ),
    };
  } catch (error) {
