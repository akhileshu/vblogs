...
> => {
  try {
    const { data: {id,...validatedData}, error } = UpdateBlogReadHistorySchema.safeParse(formData);
    if (error) return failureWithFieldErrors(error);
    const blogReadHistoryService = new BlogReadHistoryServiceImplementation(prisma);
    return { success: true, data: await blogReadHistoryService.updateBlogReadHistory(id,validatedData) };
  } catch (error) {
