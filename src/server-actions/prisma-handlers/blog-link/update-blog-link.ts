...
> => {
  try {
    const { data: {id,...validatedData}, error } = UpdateBlogLinkSchema.safeParse(formData);
    if (error) return failureWithFieldErrors(error);
    const blogLinkService = new BlogLinkServiceImplementation(prisma);
    return { success: true, data: await blogLinkService.updateBlogLink(id,validatedData) };
  } catch (error) {
