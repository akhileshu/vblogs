...
export const deleteVideoSectionHandler = async (
  ...
  Response<
      VideoSectionServiceReturnType<"deleteVideoSection">
  >
> => {
  try {
    const { data: validatedData, error } = DeleteVideoSectionSchema.safeParse(Object.fromEntries(formData.entries()));
    if (error)
      return failure("Invalid ID format");
    const videoSectionService = new VideoSectionServiceImplementation(prisma);
    return { success: true, data: await videoSectionService.deleteVideoSection(validatedData.id) };
  } catch (error) {
