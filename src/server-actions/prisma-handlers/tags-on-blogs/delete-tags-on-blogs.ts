...
export const deleteTagsOnBlogsHandler = async (
  prevState: unknown,
  formData: FormData
): Promise<Response<TagsOnBlogsServiceReturnType<"deleteTagsOnBlogs">>> => {
  try {
    const { data: validatedData, error } = DeleteTagsOnBlogsSchema.safeParse(
    ...
    return {
      success: true,
      data: await tagsOnBlogsService.deleteTagsOnBlogs(validatedData.id),
    };
  } catch (error) {
